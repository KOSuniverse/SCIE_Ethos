...
def create_shared_link(path_lower: str) -> Optional[str]:
    """
    Create a shareable Dropbox link for a file with robust error handling.
    ...
    try:
        ...
        # Strategy 1: Check if file exists first
        try:
            dbx.files_get_metadata(path_lower)
            print(f"DEBUG create_shared_link: File confirmed to exist at {path_lower}")
        except dropbox.exceptions.ApiError as e:
            print(f"DEBUG create_shared_link: File not found at {path_lower}: {e}")
            return None
        
        # Strategy 2: Try to get existing shared links first
        try:
            shared_links = dbx.sharing_list_shared_links(path=path_lower, direct_only=True)
            if shared_links.links and len(shared_links.links) > 0:
                existing_url = shared_links.links[0].url
                print(f"DEBUG create_shared_link: Found existing shared link: {existing_url}")
                # Convert to direct download link if needed
                if existing_url.endswith('?dl=0'):
                    direct_url = existing_url.replace('?dl=0', '?dl=1')
                    print(f"DEBUG create_shared_link: Converted to direct download: {direct_url}")
                    return direct_url
                return existing_url
        except Exception as list_error:
            print(f"DEBUG create_shared_link: Could not list existing links: {list_error}")
        
        # Strategy 3: Create new shared link with settings
        try:
            ...
            print(f"DEBUG create_shared_link: Successfully created new link with settings: {share_url}")
            return share_url
        except dropbox.exceptions.ApiError as e:
            print(f"DEBUG create_shared_link: Settings link creation failed: {e}")
            
            # If link already exists error, try to retrieve it again
            if "shared_link_already_exists" in str(e).lower():
                try:
                    ...
                    if shared_links.links:
                        existing_url = shared_links.links[0].url
                        print(f"DEBUG create_shared_link: Retrieved existing link after retry: {existing_url}")
                        return existing_url
                except Exception:
                    pass
        
        # Strategy 4: Simple link creation without settings
        try:
        ...
        # Strategy 5: Fallback - return a manual construction
        try:
            # Get account info to construct manual link
            account_info = dbx.users_get_current_account()
            if account_info and hasattr(account_info, 'account_id'):
                # This is a last resort - may not always work
                manual_url = f"https://www.dropbox.com/s/[file_id]{path_lower.split('/')[-1]}?dl=0"
                print(f"DEBUG create_shared_link: Manual URL construction attempted: {manual_url}")
                # Don't return this as it's not reliable
        except Exception:
            pass
            
        print(f"DEBUG create_shared_link: All strategies failed for {path_lower}")
        return None
            
    except Exception as e:
        print(f"DEBUG create_shared_link: General error for {path_lower}: {e}")
        import traceback
        print(f"DEBUG create_shared_link: Full traceback: {traceback.format_exc()}")
        return None
