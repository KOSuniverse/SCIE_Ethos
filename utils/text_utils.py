...
def extract_text_for_metadata(file_id, max_ocr_pages=5):
    """
    ...
    """
    try:
        # Determine file type from file_id (you might need to get filename from Drive metadata)
        # For now, we'll try to determine from the downloaded content
        file_stream = download_file(file_id)
        
        # Try to determine file type from stream content or filename
        # This is a simplified approach - you might want to get actual filename from Drive metadata
        try:
            # Try Excel first
